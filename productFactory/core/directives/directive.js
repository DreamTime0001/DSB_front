        ...
        return {
            ...
            controller: function ($scope, $element) {
                $scope.createHtml = function () {
                    if ($scope.page.pages > 1) {
                        ...
                        if ($scope.page.pages < 5) {
                            ...
                            if ($scope.page.pageNum <= 4) {
                                ...
                                var maxNum = Number($scope.page.pageNum) + 2;
                                if (maxNum > $scope.page.pages) {
                                    maxNum = $scope.page.pages;
                                }
                                var minNum = Number($scope.page.pageNum) - 2;
                    ...
                    if ($scope.page.pageSize == 20) {
                        str += '<select class="form-control " style="display: inline-block;width:60px;vertical-align: top;margin:0 0 0 10px;height:34px;border:1px solid #ddd;color:#999;text-align: center" ><option value="20" selected>20</option><option value="50">50</option><option value="100">100</option></select>';
                    } else if ($scope.page.pageSize == 50) {
                        str += '<select class="form-control " style="display: inline-block;width:60px;vertical-align: top;margin:0 0 0 10px;height:34px;border:1px solid #ddd;color:#999;text-align: center" ><option value="20">20</option><option value="50" selected>50</option><option value="100">100</option></select>';
                    } else if ($scope.page.pageSize == 100) {
                        str += '<select class="form-control " style="display: inline-block;width:60px;vertical-align: top;margin:0 0 0 10px;height:34px;border:1px solid #ddd;color:#999;text-align: center" ><option value="20">20</option><option value="50">50</option><option value="100" selected>100</option></select>';
                    }
                    $element.html(str);
                    $scope.bindEvent();
                    $scope.selectChange();
                ...
                $scope.bindEvent = function () {
                    $element.find('a').parent().on('click', function () {
                        ...
                        var pageNo = Number($scope.page.pageNum);
                        if ($.trim(text) == '上一页') {
                ...
                $scope.$watch('page.pageNum', function (newVal, oldVal) {
                    var currentObj = $scope.page.url;
                    localStorage.setItem(currentObj, JSON.stringify($scope.page));
                    return $scope.createHtml();
                })
            }
        }
