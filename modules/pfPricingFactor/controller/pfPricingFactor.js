(function () {
    ...
    angular.module('app.controllers')
        .controller('pfPricingFactorController', ['$api', '$timeout', '$upload', '$http', '$state', '$scope', '$defaultConfig', '$layer', function ($api, $timeout, $upload, $http, $state, $scope, $defaultConfig, $layer) {
            var vm = this;
            $timeout(function () {
                initMenu();
            }, 100);
            ...
            vm.getPagedDataAsync = function () {
                $api.post('/productfactory/priceConfigList', angular.toJson(vm.query), function (result) {
                    if (result.code === 0) {
...
//---------------------------------------------------------------------------------------分割线-----------------------------------------------------------------------------------------//
            ...
            vm.updeFactorPrecent = function (obj) {
                ...
                if (/^(([0-9]+\.[0-9]*[1-9][0-9]*)|([0-9]*[1-9][0-9]*\.[0-9]+)|([0-9]*[1-9][0-9]*))$/.test(obj.factor)) {
                    $api.post('/productfactory/updatePriceConfig', angular.toJson({factor: obj.factor,id:obj.id}), function (result) {
                        if (result.code === 0) {
                            layer.msg(result.msg, {time: 1000});
                            obj.checked = false;
                        }
