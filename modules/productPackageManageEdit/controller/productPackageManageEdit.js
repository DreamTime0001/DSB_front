(function() {
  ...
  angular
    ...
    .controller("productPackageManageEditController", [
      "$api",
      "$timeout",
      "$upload",
      "$http",
      ...
      function(
        ...
        //获取数据
        $api.get("/tb/productWrap/detail/" + $stateParams.id, function(result) {
          if (result.code == 0) {
            vm.productData = result.data;
            vm.productData.goodKindChecked = [];
            //添加理赔互斥数组
            for (var i = 0; i < vm.productData.products.length; i++) {
              if (vm.productData.products[i].mutex != "") {
                vm.productData.products[
                  i
                ].mutexedList = vm.productData.products[i].mutex.split(",");
              } else {
            ...
            //获取产品详情
            $timeout(function() {
              editor.txt.html(vm.productData.description);
            }, 500);
            //商品类别数据绑定
            vm.tempGoodKind = vm.productData.goodKind.split(",");
            for (var i = 0; i < vm.tempGoodKind.length; i++) {
              for (var j = 0; j < vm.goodKindList.length; j++) {
                if (vm.tempGoodKind[i] == vm.goodKindList[j].name) {
                  vm.goodKindList[j].checked = true;
                }
              }
            }
            //投保方式数据绑定
            vm.tempOrderWay = vm.productData.tbProductWrapExtend.orderWay.split(
              ","
            );
            for (var i = 0; i < vm.tempOrderWay.length; i++) {
              for (var j = 0; j < vm.insuranceInformation.length; j++) {
                if (vm.tempOrderWay[i] == vm.insuranceInformation[j].type) {
                  vm.insuranceInformation[j].checked = true;
                }
              }
            }
            //投保必填项数据绑定
            vm.tempOrderInsurance = vm.productData.tbProductWrapExtend.orderInsurance.split(
              ","
            );
            for (var i = 0; i < vm.tempOrderInsurance.length; i++) {
              for (var j = 0; j < vm.insureItems.length; j++) {
                if (vm.tempOrderInsurance[i] == vm.insureItems[j].type) {
                  vm.insureItems[j].checked = true;
                }
              }
            }
            //理赔方式数据绑定
            vm.tempApplyInsurance = vm.productData.tbProductWrapExtend.applyInsurance.split(
              ","
            );
            for (var i = 0; i < vm.tempApplyInsurance.length; i++) {
              for (var j = 0; j < vm.claimList.length; j++) {
                if (vm.tempApplyInsurance[i] == vm.claimList[j].type) {
                  vm.claimList[j].checked = true;
                }
              }
            }
            //图片+视频数据绑定
            vm.tempApplyInsurancePicVedio = vm.productData.tbProductWrapExtend.applyInsurancePicVedio.split(
              ","
            );
            for (var i = 0; i < vm.tempApplyInsurancePicVedio.length; i++) {
              for (var j = 0; j < vm.videoImgList.length; j++) {
                if (
                  vm.tempApplyInsurancePicVedio[i] == vm.videoImgList[j].docName
                ) {
            ...
            //文字介绍数据绑定
            vm.tempApplyInsuranceDoc = vm.productData.tbProductWrapExtend.applyInsuranceDoc.split(
              ","
            );
            for (var i = 0; i < vm.tempApplyInsuranceDoc.length; i++) {
              for (var j = 0; j < vm.fontList.length; j++) {
                if (vm.tempApplyInsuranceDoc[i] == vm.fontList[j].docName) {
                  vm.fontList[j].checked = true;
                }
              }
            }
            ...
            $api.get("/tb/productWrap/productTemplateList", function(result) {
              if (result.code == 0) {
                ...
                console.log(vm.productTemplateIdList);
                console.log(vm.productData.products);
                for (var i = 0; i < vm.productData.products.length; i++) {
                  for (var j = 0; j < vm.productTemplateIdList.length; j++) {
                    if (
                      vm.productData.products[i].productTemplateId ==
                      vm.productTemplateIdList[j].id
                    ) {
                      vm.productData.products[i].showName =
                        vm.productTemplateIdList[j].id +
                        "-" +
                        vm.productTemplateIdList[j].product_name;
                    }
                  }
                }
          ...
          for (var i = 0; i < vm.productData.products.length; i++) {
            for (var j = 0; j < vm.productTemplateIdList.length; j++) {
              if (
                ...
                vm.mutexList.push({
                  id: vm.productTemplateIdList[j].id,
                  showName:
                    vm.productTemplateIdList[j].id +
                    "-" +
                    vm.productTemplateIdList[j].product_name
